...
def quiz(capitals_dict):
	...
	incorrect_answers = []
	counter = 0
	while len(capitals_dict) > 0 and counter <= 10:
		capital_asked = random.choice(capitals_dict.keys())
		correct_answer = capitals_dict.get(capital_asked)
		answer = raw_input("What is the capital of " + capital_asked + "? ")
		if answer == correct_answer:
			...
			del capitals_dict[capital_asked]
			print "Sorry, that's not right. The correct answer is " + correct_answer + "."
		counter = counter + 1
		# print counter	
	print "Quiz over! You've missed", len(incorrect_answers), "countries out of", len(correct_answers_list) + len (incorrect_answers)

# How to make the test_kickoff dictionary printing nicer? (remove ''s)
